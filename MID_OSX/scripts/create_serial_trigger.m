function [TRIGGER_PORT,PLACE] = create_serial_trigger(TRIGGER_PORT, TRIGGER_MARKER)
...
try
    ...
    TRIGGER_PORT = input('Which port to use instead? (Enter full path without quote)\n','s');
    % try again
    TRIGGER_PORT = create_serial_trigger(TRIGGER_PORT);
end
end